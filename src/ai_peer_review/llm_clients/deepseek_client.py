...
class DeepSeekClient(BaseLLMClient):
    """Client for DeepSeek API via Together.ai."""

    def __init__(self, model: str, api_key: Optional[str] = None):
        """
        Initialize DeepSeek client using Together.ai API.

        Args:
            model: Model name
            api_key: Together.ai API key (optional, will use TOGETHER_API_KEY env var if not provided)
        """
        self.model = "deepseek-ai/deepseek-r1"  # Together.ai model name for DeepSeek
    ...
    def generate(self, prompt: str) -> str:
        """
        Generate response from DeepSeek model via Together.ai.
